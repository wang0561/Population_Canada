...
public class TableViewOfGEO<PopulationProperties> extends TableViewAbstraction<PopulationProperties> {
	...
	final String[] colnames = {"Year","Gender", "AgeGroup", "Number","Median"};
	final String[] colValues = {"Year","Gender","Agegroup","Number","Median"};
	
	public TableViewOfGEO(String value, List<PopulationNum> numbers, List<PopulationMedian> medians) {
	...
	private ObservableList<PopulationProperties> getObservableList() {
		...
		List<PopulationMedian> meds = medians.stream().filter(e->e.getGEO().equals(value)).collect(Collectors.toList());
		...
		for(PopulationNum populationNum : nums ) {
			...
			data.add((PopulationProperties) new populcation.age.gender.canada.properties_table_column.PopulationProperties(populationNum.getYear(),populationNum.getGender(),populationNum.getAgeGroup(),
